...
import com.transferer.shared.outbox.OutboxEvent;
...
/**
 * A test EventBus that publishes each event twice to verify idempotency.
 * Each event is published, then immediately published again to ensure
 * the system handles duplicate events correctly.
 */
public class DuplicateEventBus implements TransactionalEventBus {
    
    private static final Logger logger = LoggerFactory.getLogger(DuplicateEventBus.class);
    ...
    public DuplicateEventBus(OutboxEventRepository outboxEventRepository, ObjectMapper objectMapper) {
    ...
    public Mono<Void> publish(DomainEvent<?> event) {
        return getOutboxEvent(event)
                ...
                .then()
                // Publish the same event again immediately to test idempotency
                .then(getOutboxEvent(event)
                        .flatMap(outboxEventRepository::save)
                        .doOnNext(savedEvent -> {
                            // Emit to reactive stream again
                            eventSink.tryEmitNext(event);
                            notifySubscribers(event);
                        })
                        .then()
